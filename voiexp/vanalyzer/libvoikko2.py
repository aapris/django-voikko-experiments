...
class Sentence:
    ...
    def __repr__(self):
        return (u"<" + self.sentenceText + u"," + \
                Sentence._TYPE_NAMES[self.nextStartType] + u">").encode(
            "UTF-8")
...
class Voikko(object):
    ...
    def __init__(self, language, path=None):
        ...
        if error.value != None:
            ...
            raise VoikkoException(
                u"Initialization of Voikko failed: " + unicode(error.value,
                                                               "UTF-8"))
    ...
    def __setBoolOption(self, option, value):
        result = self.__lib.voikkoSetBooleanOption(self.__handle, option,
                                                   _boolToInt(value))
        if result == 0:
            raise VoikkoException(u"Could not set boolean option " + str(
                option) + u" to value " + str(value) + u".")
    ...
    def __getGrammarError(self, cGrammarError, language):
        ...
        gError.shortDescription = unicode(string_at(cErrorShortDescription),
                                          "UTF-8")
